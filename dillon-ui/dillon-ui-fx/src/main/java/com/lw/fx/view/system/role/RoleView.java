...
//                            box.setSpacing(7);
    ...
    private void showRoleAssignMenuForm(RoleRespVO respVO) {
        ...
        ViewTuple<RoleAssignMenuFormView, RoleAssignMenuFormViewModel> load = FluentViewLoader.fxmlView(RoleAssignMenuFormView.class).load();
        load.getViewModel().getSimpleMenuList(respVO);

        WFXGenericDialog dialog = new WFXGenericDialog();
        dialog.setHeaderIcon(FontIcon.of(Feather.INFO));
        dialog.setHeaderText("菜单权限");
        dialog.addActions(
                Map.entry(new Button("取消"), event -> dialog.close()),
                Map.entry(new Button("确定"), event -> {
                    ProcessChain.create()
    ...
    private void showRoleDataPermissionForm(RoleRespVO respVO) {
        ...
        ViewTuple<RoleDataPermissionFormView, RoleDataPermissionFormViewModel> load = FluentViewLoader.fxmlView(RoleDataPermissionFormView.class).load();
        load.getViewModel().getSimpleDeptList(respVO);

        WFXGenericDialog dialog = new WFXGenericDialog();
        dialog.setHeaderIcon(FontIcon.of(Feather.INFO));
        dialog.setHeaderText("数据权限");
        dialog.addActions(
                Map.entry(new Button("取消"), event -> dialog.close()),
                Map.entry(new Button("确定"), event -> {
                    ProcessChain.create()
                            .addSupplierInExecutor(() -> {
                                return load.getViewModel().assignRoleDataScope();
                            })
                            .addConsumerInPlatformThread(r -> {
                                if (r.isSuccess()) {
                                    MvvmFX.getNotificationCenter().publish("message", "操作成功", MessageType.SUCCESS);
    ...
    private void showDelDialog(RoleRespVO respVO) {
        ...
        dialog.addActions(
                ...
                Map.entry(new Button("确定"), event -> {
                    ProcessChain.create()
                            .addSupplierInExecutor(() -> {
                                return Request.connector(RoleFeign.class).deleteRole(respVO.getId());
                            })
                            .addConsumerInPlatformThread(r -> {
                                if (r.isSuccess()) {
